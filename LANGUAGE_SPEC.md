...
**[R-EXTERN-1]** Extern declarations support scalar and pointer parameters/results, including
function pointers with supported signatures. **[R-EXTERN-2]** Aggregate values (structs, enums,
tuples, fixed arrays, slices, and Lambdas) are rejected with
`BPL_EXTERN_ABI_UNSUPPORTED`; use C wrappers with pointers/output buffers. **[R-EXTERN-3]** Matching
LLVM aggregate layout does not implement platform-specific C argument/result
classification. **[R-EXTERN-4]** Direct extern variadic calls also reject aggregate arguments,
