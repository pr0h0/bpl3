...
| `bpl list --json` / `bpl list --tree --json` | Package inspection reports with `schemaVersion`, `check: "package-list"` or `check: "package-list-tree"`, `success`, `scope`, and the existing installed package summaries or dependency tree data; unsafe package-root validation failures return `success: false`, `packages: []` or `tree: []`, and `error`. List and tree duplicate installed package names return `errorCode: "BPL_PACKAGE_DUPLICATE_INSTALLED"`. List and tree duplicate failures also include `issuesFound`, `issueKinds`, and compact `issues` entries with `kind: "duplicate-installed-package"`. |
