...
for (const bits of [8, 16, 32, 64]) {
  for (const signed of [true, false]) {
    ...
    const label = ({ 8: "char", 16: "short", 32: "int", 64: "long" } as Record<number, string>)[bits]!;
    canonical[name] = {
      canonicalName: name, kind: "integer", bits, signed, llvmType: `i${bits}`,
      debugName: signed ? (bits === 8 ? "signed char" : label) : `unsigned ${label}`,
      debugEncoding: bits === 8 ? (signed ? 6 : 8) : (signed ? 5 : 7),
    };
  }
}
canonical.i1 = { canonicalName: "i1", kind: "boolean", bits: 1, signed: false, llvmType: "i1", debugName: "bool", debugEncoding: 2 };
canonical.f32 = { canonicalName: "f32", kind: "float", bits: 32, signed: true, llvmType: "float", debugName: "float", debugEncoding: 4 };
canonical.double = { canonicalName: "double", kind: "float", bits: 64, signed: true, llvmType: "double", debugName: "double", debugEncoding: 4 };

export const PRIMITIVE_ALIASES: Readonly<Record<string, string>> = Object.freeze({
  int: "i32", uint: "u32", float: "double", f64: "double", bool: "i1",
  char: "i8", uchar: "u8", short: "i16", ushort: "u16", long: "i64", ulong: "u64",
});
...
export const PRIMITIVE_TYPES: Readonly<Record<string, PrimitiveTypeInfo>> = Object.freeze(types);
export const PRIMITIVE_NAMES = Object.keys(PRIMITIVE_TYPES);
export const PRIMITIVE_INTEGER_NAMES = PRIMITIVE_NAMES.filter((name) => PRIMITIVE_TYPES[name]!.kind === "integer");
