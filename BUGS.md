...
| BUG-133 | Inline Assembly | Variables used in inline assembly blocks are reported as "unused" by the linter. | Open | Inline asm variable tracking doesn't recognize uses of `a`, `b`, etc. in asm constraints. |
| BUG-134 | Inline Assembly | LLVM IR inline assembly generates incorrect type references for pointer parameters. | Open | Error: "'%value_ptr.0' defined with type 'ptr' but expected 'i32'" in shift operations. |
| BUG-135 | IO Library | `IO.printInt`, `IO.printFloat`, `IO.printBool` add implicit newlines unlike typical print functions. | Open | Consider adding `IO.printIntRaw()` or changing naming convention to `IO.printlnInt()` for clarity. |
| BUG-136 | Codegen | Method calls on array-element-derived pointers (e.g., `arr[i].method()` or `ptr.method()` where `ptr = &arr[i]`) cause runtime SIGSEGV at addresses like 0x18, 0x20, 0x28, 0x38. | Fixed | **Root Cause**: Vtable was not initialized for: (1) stack-allocated struct arrays, (2) heap-allocated structs (via malloc). **Fix**: Three-part solution: (a) Initialize vtable for stack arrays at declaration time (StatementGenerator.generateArrayVTableInitialization), (b) Use direct dispatch (not virtual) for `init` methods so they can be called on uninitialized vtable memory (CallExpressionGenerator), (c) Inject vtable initialization at the start of `init` method bodies so subsequent virtual calls work (StatementGenerator.generateFunction). **Note**: Heap-allocated arrays (via malloc) still require manual workaround using free functions like `initStructAt(ptr)` instead of `ptr.init()`. |
| BUG-137 | Runtime/Vtable | mini_database_engine crashes during INSERT/SELECT operations due to uninitialized vtable in heap-allocated struct arrays and clone operations. | Fixed | **Root Cause**: BPL structs with methods have a hidden vtable pointer that must be initialized. When allocating struct arrays via `malloc` and using free helper functions (e.g., `initValueAt(&arr[i])`) instead of calling the struct's `.init()` method directly (e.g., `arr[i].init()`), the vtable pointer remains uninitialized (null or garbage). Later virtual method calls on these objects crash at low addresses like 0x28. **Fix**: (1) Replace all `initValueAt(&arr[i])` calls with `arr[i].init()` since the compiler uses direct dispatch for init() and automatically injects vtable initialization inside init method bodies. (2) In Value.clone(), add `dest.init()` call before copying fields to ensure destination vtable is initialized. (3) In Table.init(), use `this.columns[c].init()` instead of `initColumnDefAt(&this.columns[c])` for ColumnDef array initialization. **Lesson**: For heap-allocated struct arrays, always call `.init()` method directly on array elements; never use free helper functions that merely set field values without initializing the vtable. |
