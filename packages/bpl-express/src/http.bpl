...
struct Response {
    ...
    frame send(this: *Response, body: string) {
        ...
        if (code == 404) { status_msg = "Not Found"; }
        if (this.status_code == 500) { status_msg = "Internal Server Error"; }
        if (this.status_code == 201) { status_msg = "Created"; }
        if (this.status_code == 400) { status_msg = "Bad Request"; }
        if (this.status_code == 204) { status_msg = "No Content"; }
