...
const invalidCases = [
  ...
  { type: "Float32Pair", input: "[1.25]" },
  { type: "Array<f32>", input: "[1.25]" },
  ...[
    '{"id":1,}',
    '{"id":1 "extra":2}',
    '{"extra":}',
  ...
  const source = `
    ...
    frame main() ret int {
      ${invalidCases
        .map(
          ({ type, input }, index) => `
        local value${index}: *${type} = JSON.parse<${type === "Pair" ? "int[2]" : type === "Float32Pair" ? "f32[2]" : type}>(${JSON.stringify(input)});
        if (value${index} != nullptr) { JSON.free<${type === "Pair" ? "int[2]" : type === "Float32Pair" ? "f32[2]" : type}>(value${index}); return ${index + 1}; }
      `,
        )
        .join("\n")}
