...
frame main() ret int {
    ...
    local unique: Array<int> = Algorithm.unique(&withDupes);
    printIntArray(&unique, "Unique values");

    # Test count
    printf("\n--- Count ---\n");
    printf("Count of 3 in withDupes: %d\n", Algorithm.count(&withDupes, 3));
    printf("Count of 5 in withDupes: %d\n", Algorithm.count(&withDupes, 5));

    # Test range generation
    printf("\n--- Range Generation ---\n");
    local range1: Array<int> = Algorithm.range(0, 5);
    printIntArray(&range1, "range(0, 5)");

    local range2: Array<int> = Algorithm.rangeStep(0, 10, 2);
    printIntArray(&range2, "range(0, 10, step=2)");

    local range3: Array<int> = Algorithm.rangeStep(10, 0, -2);
    ...
    b.push(5);
    b.push(6);
    local merged: Array<int> = Algorithm.merge(&a, &b);
    printIntArray(&merged, "Merged [1,2,3] + [4,5,6]");

    # Test equals
    printf("\n--- Equality Check ---\n");
    local c: Array<int> = a.clone();
    printf("a equals c (clone): %d\n", cast<int>(Algorithm.equals(&a, &c)));
    printf("a equals b: %d\n", cast<int>(Algorithm.equals(&a, &b)));
    ...
    printf("min: %.1f\n", Algorithm.min(&floats));
    printf("max: %.1f\n", Algorithm.max(&floats));
    printf("sum: %.1f\n", Algorithm.sum(&floats));
    printf("average: %.2f\n", Algorithm.average(&floats));

    Algorithm.sortAsc(&floats);
