        ...
        if (dx != 0 || dy != 0) {
            ...
            if (!attacked) {
                ...
                match (target_tile.kind) {
                    TileType.Wall => { blocked = true; },
                    TileType.Door(open) => {
                        if (!open) blocked = true;
                    },
                    TileType.Floor => { blocked = false; }
                }
            ...
            loop (local i: int = 0; i < monsters.len(); i = i + 1) {
                ...
                if (m.is_alive()) {
                    ...
                    if (player.x > m.x) mdx = 1;
                    if (player.x < m.x) mdx = -1;
                    if (player.y > m.y) mdy = 1;
                    if (player.y < m.y) mdy = -1;
                    ...
                    if (mx == player.x && my == player.y) {
                        ...
                        match (mt.kind) {
                            TileType.Wall => { mblocked = true; },
                            TileType.Door(open) => { 
                                if (!open) mblocked = true; 
                            },
                            TileType.Floor => { mblocked = false; }
                        }
                        if (!mblocked) {
                            m.x = mx;
                            m.y = my;
