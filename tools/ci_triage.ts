...
const STATEMENT_SEMANTIC_GUARD_STEP_PATTERN = new RegExp(
  [
    ...
    "statement semantic guard",
    "Missing type annotation for variable",
    "Variable '.*' is already declared in this scope",
    "Integer overflow: value .* does not fit in type",
    ...
    "Ensure the value is within the range of",
...
const INTRINSIC_CALL_DIAGNOSTIC_STEP_PATTERN = new RegExp(
  [
    "BPL_INTRINSIC_GENERIC_ARITY_MISMATCH",
    "BPL_INTRINSIC_ARGUMENT_COUNT_MISMATCH",
    "intrinsic call diagnostics",
    "intrinsic call diagnostic",
    "intrinsic call semantic",
    "Intrinsic __type_(?:id|info) requires exactly 1 generic argument",
    "Intrinsic __type_(?:id|info) accepts no arguments",
    "Use __type_(?:id|info)<T>\\(\\) with exactly one type argument",
    "Call __type_(?:id|info)<T>\\(\\) without value arguments",
  ].join("|"),
  "i",
);
...
const TYPE_QUERY_DIAGNOSTIC_STEP_PATTERN = new RegExp(
  [
    "BPL_TYPE_QUERY_ENUM_NOT_FOUND",
    "BPL_TYPE_QUERY_TYPE_NOT_FOUND",
    "type-query diagnostics",
    "type-query diagnostic",
    "Type-query failures",
    "Cannot find enum",
    "Unknown type ['\"].*['\"]",
    "Unknown type: .*",
    "The type ['\"].*['\"] in match<.*> is not a defined enum",
    "The type ['\"].*['\"] in match<.*> is not defined",
    "in ['\"]is['\"] expression is not a defined enum",
  ].join("|"),
  "i",
);
...
const EXCLUSIVE_STEP_REPRO_COMMANDS: Array<[RegExp, string]> = [
  ...
  [
    INDEX_EXPRESSION_MISUSE_STEP_PATTERN,
  ...
  [
    STATEMENT_SEMANTIC_GUARD_STEP_PATTERN,
  ...
  [
    INTRINSIC_CALL_DIAGNOSTIC_STEP_PATTERN,
  ...
  [
    TYPE_QUERY_DIAGNOSTIC_STEP_PATTERN,
